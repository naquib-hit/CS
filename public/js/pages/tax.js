...
let selectedRows = [],
    data = null,
    fetchUrl = `${window.location.origin}/taxes/get`;
// INIT DATA
...
const setTable = async data => {
    ...
    Array.from(data.data, item => {
        ...
        cell_2.dataset.name = 'tax_name';
        cell_2.innerText = item['tax_name'];
        cell_2.classList.add('ps-1');
        // Column 4
        const cell_3 = row.insertCell(3);
        cell_3.dataset.name = 'tax_amount';
        cell_3.innerText = item['tax_amount'];
        ...
        cell_4.innerHTML =  `<span class="d-flex flex-nowrap flex-grow-0 align-items-center">` +
                                `<a type="button" class="btn btn-sm btn-info btn-circle p-0 m-0 edit_data" data-bs-toggle="tooltip" data-bs-title="Edit" href="${window.location.origin}/taxes/${item.id}/edit">` + 
...
const deleteConfirmation = e => {
    ...
    .then(t => {
        ...
        loading();
        fetch(`${window.location.origin}/taxes/${props.id}`, {
            method: 'DELETE',
            headers: {
...
const filterData = async e => {
    ...
    try 
    {
        fetchUrl = `${window.location.origin}/taxes/get?` + params;
        data = await getData(fetchUrl);
        await setTable(data);
    } 
    catch (error) 
...
const deleteAllRows = async (e, opt) => {
    ...
    .then(async t => {
        ...
        if(filter.s_tax_name)
            params.tax_name = filter.s_tax_name;
