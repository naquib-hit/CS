...
(async () => {
    ...
    frmSearch.addEventListener('reset', async e => {
        ...
        fetchUrl = `${window.location.origin}/reccuringInvoices/get`;
        data = await getData(fetchUrl);
    ...
    Array.from(data.data, item => {
        ...
        if(item['invoice_status'] !== 1) 
            cell_6.innerHTML += `<span class="d-flex flex-nowrap flex-grow-0 align-items-center">` +
                                `<a type="button" class="btn btn-sm btn-success btn-circle p-0 m-0 edit_data" data-bs-toggle="tooltip" data-bs-title="Edit" href="${window.location.href}/${item.id}">` + 
                                    `<i class="fas fa-eye font-reset"></i>` +
                                `</a>` +
                                `<a type="button" class="btn btn-sm btn-info btn-circle p-0 m-0 ms-1 edit_data" data-bs-toggle="tooltip" data-bs-title="Edit" href="${window.location.href}/${item.id}/edit">` + 
        ...
        cell_6.innerHTML += `<span class="d-flex flex-nowrap flex-grow-0 align-items-center">` +
                                `<a type="button" class="btn btn-sm btn-success btn-circle p-0 m-0 edit_data" data-bs-toggle="tooltip" data-bs-title="Edit" href="${window.location.origin}/recurringInvoices/${item.id}">` + 
                                    `<i class="fas fa-eye font-reset"></i>` +
                                `</a>`;
...
const getProducts = async () => {
    try
    {
        const f = await fetch(`${window.location.origin}/invoices/products`);
...
const deleteConfirmation = e => {
    ...
    .then(t => {
        ...
        loading();
        fetch(`${window.location.origin}/recurringInvoices/${props.id}`, {
            method: 'DELETE',
            headers: {
...
const filterData = async e => {
    ...
    try 
    {
        fetchUrl = `${window.location.origin}/recurringInvoices/get?` + params;
        data = await getData(fetchUrl);
        await setTable(data);
    } 
    catch (error) 
