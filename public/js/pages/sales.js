...
const setTable = async data => {
    ...
    Array.from(data.data, item => {
        ...
        cell_2.dataset.name = 'sales_code';
        cell_2.innerText = item['sales_code'];
        cell_2.classList.add('ps-1');
        // Column 4
        const cell_3 = row.insertCell(3);
        cell_3.dataset.name = 'sales_name';
        cell_3.innerText = item['sales_name'];
        cell_3.classList.add('ps-1');
        // Column 5
        const cell_4 = row.insertCell(4);
        cell_4.dataset.name = 'sales_email';
        cell_4.innerText = item['sales_email'];
        ...
        cell_5.innerHTML =  `<span class="d-flex flex-nowrap flex-grow-0 align-items-center">` +
                                `<a type="button" class="btn btn-sm btn-info btn-circle p-0 m-0 edit_data" data-bs-toggle="tooltip" data-bs-title="Edit" href="${window.location.origin}/sales/${item.id}/edit">` + 
...
const deleteConfirmation = e => {
    ...
    .then(t => {
        ...
        loading();
        fetch(`${window.location.origin}/sales/${props.id}`, {
            method: 'DELETE',
            headers: {
...
const deleteAllRows = async (e, opt) => {
    ...
    .then(async t => {
        ...
        if(filter.s_sales_code)
            params.sales_code = filter.s_sales_code;
        if(filter.s_sales_name)
            params.sales_name = filter.s_sales_name;
        if(filter.s_sales_email)
            params.sales_email = filter.s_sales_email;
