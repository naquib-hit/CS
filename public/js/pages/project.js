...
let selectedRows = [],
    data = null,
    fetchUrl = `${window.location.origin}/projects/get`;
// INIT DATA
...
const setTable = async data => {
    ...
    Array.from(data.data, item => {
        ...
        cell_1.dataset.name = 'id';
        cell_1.innerText = item.id;
        ...
        cell_5.innerHTML =  `<span class="d-flex flex-nowrap flex-grow-0 align-items-center">` +
                                `<a type="button" class="btn btn-sm btn-info btn-circle p-0 m-0 edit_data" data-bs-toggle="tooltip" data-bs-title="Edit" href="${window.location.origin}/projects/${item.id}/edit">` + 
...
const deleteConfirmation = e => {
    ...
    .then(t => {
        ...
        loading();
        fetch(`${window.location.origin}/projects/${props.id}`, {
            method: 'DELETE',
            headers: {
...
const filterData = async e => {
    ...
    try 
    {
        fetchUrl = `${window.location.origin}/projects/get?` + params;
        data = await getData(fetchUrl);
        await setTable(data);
    } 
    catch (error) 
