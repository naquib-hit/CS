...
let selectedRows = [],
    data = null,
    fetchUrl = `${window.location.origin}/users/get`;
// INIT DATA
...
(async () => {
    ...
    // delete all table
    deleteAllBtn.addEventListener('click', async e => await deleteAllRows(e, {}));
    // seacrh
    ...
    frmSearch.addEventListener('reset', async e => {
        document.getElementById('loading-table').classList.remove('d-none');
        fetchUrl = `${window.location.origin}/user/get`;
        data = await getData(fetchUrl);
        await setTable(data);
        document.getElementById('loading-table').classList.add('d-none');
    }); 
...
const setTable = async data => {
    ...
    Array.from(data.data, item => {
        ...
        cell_2.dataset.name = 'username';
        cell_2.innerText = item['username'];
        cell_2.classList.add('ps-1');
        // Column 4
        const cell_3 = row.insertCell(3);
        cell_3.dataset.name = 'fullname';
        cell_3.innerText = item['fullname'];
        cell_3.classList.add('ps-1');
        // Column 5
        const cell_4 = row.insertCell(4);
        cell_4.dataset.name = 'email';
        cell_4.innerText = item['email'];
        ...
        cell_5.innerHTML =  `<span class="d-flex flex-nowrap flex-grow-0 align-items-center">` +
                                `<a type="button" class="btn btn-sm btn-info btn-circle p-0 m-0 edit_data" data-bs-toggle="tooltip" data-bs-title="Edit" href="${window.location.origin}/users/${item.id}/edit">` + 
...
const deleteConfirmation = e => {
    ...
    .then(t => {
        ...
        loading();
        fetch(`${window.location.origin}/users/${props.id}`, {
            method: 'DELETE',
            headers: {
...
const filterData = async e => {
    ...
    let params = new URLSearchParams(Object.fromEntries(frm.entries()));

    try 
    {
        fetchUrl = `${window.location.origin}/users/get?` + params;
        data = await getData(fetchUrl);
        await setTable(data);
    } 
    catch (error) 
...
const deleteAllRows = async (e, opt) => {
    ...
    .then(async t => {
        ...
        if(filter.s_username)
            params.username = filter.s_username;
        if(filter.s_fullname)
            params.fullname = filter.s_fullname;
        if(filter.s_email)
            params.email = filter.s_email;
