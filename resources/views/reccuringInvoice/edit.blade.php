...
@section('content')
    ...
    <div class="row h-100 justify-content-center">
        <form class="col-12" 
              ...
              action="{{ route('reccuringInvoices.update', ['reccuringInvoice' => $invoice['id']]) }}"
              method="POST"
                ...
                <div class="col-12">
                    <div class="card fadeIn3 fadeInBottom">
                        ...
                        <div class="card-body row">
                            ...
                            <div class="col-12 col-lg-4">
                                <div class="input-group input-group-static @error('invoice_next') is-invalid @enderror mt-3">
                                    <label class="form-label">{{ __('invoice.form.fields.next') }}<span class="text-danger">*</span></label>
                                    <input type="text" class="form-control" name="invoice_next" value="{{ old('invoice_next') ?? $invoice['next_date'] }}">
                                </div>
                                @error('invoice_due')
                                    <small class="text-danger">{{ $message }}</small>
                                @enderror
                            ...
                            <div class="col-12 col-lg-4">
                                <div class="input-group input-group-static flex-nowrap flex-column @error('invoice_interval') is-invalid @enderror mt-3">
                                    <label class="form-label">{{ __('invoice.form.fields.interval') }}  <span class="text-danger">*</span></label>
                                    <div class="d-flex flex-nowrap">
                                    <select class="form-control" name="invoice_interval" value="{{ old('invoice_interval') ?? $invoice['frequency'] }}">
                                        <option value="daily">{{ __('Harian') }}</option>
                                        <option value="weekly">{{ __('Mingguan') }}</option>
                                        <option value="monthly">{{ __('Bulanan') }}</option>
                                        <option value="2_month">{{ __('2 Bulan') }}</option>
                                        <option value="3_month">{{ __('3 Bulan') }}</option>
                                        <option value="6_month">{{ __('6 Bulan') }}</option>
...
import { Autocomplete } from "{{ asset('vendor/autocomplete/autocomplete.js') }}";
    ...
    // Set Next Date Value
    const nextDate = document.querySelector('input[name="invoice_next"]'),
          startDate = document.querySelector('input[name="invoice_date"]'),
          interval = document.querySelector('select[name="invoice_interval"]');

    var ft = flatpickr(startDate, {
                defaultDate: new Date()
            });      

    const setNextDate = (initValue, intrvl) => {
        var nextValue = new Date(initValue.getFullYear(), initValue.getMonth(), initValue.getDate());
        // cek interval value
        switch(intrvl.value)
        {
            case 'daily':
                nextValue.setDate(initValue.getDate() + 1);
                break;
            case 'weekly':
                var timer = nextValue.setTime(initValue.getTime() + (7 * 24 * 60 * 60 * 1000));
                nextValue = new Date(timer);
                break;
            case 'monthly':
                nextValue.setMonth(initValue.getMonth() + 1);
                break;
            case '2_month':
                nextValue.setMonth(initValue.getMonth() + 2);
                break;
            case '3_month':
                nextValue.setMonth(initValue.getMonth() + 3);
                break;
            case '6_month':
                nextValue.setMonth(initValue.getMonth() + 6);
                break;
            case 'yearly':
                nextValue.setFullYear(initValue.getFullYear() + 1);
                break;
        }

        //console.log(nextValue);

        var nMonth = nextValue.getMonth() + 1;

        console.log(nMonth);

        var year = nextValue.getFullYear(),
            month = (nMonth < 10 ? '0' + nMonth : nMonth),
            date = (nextValue.getDate() < 10 ? '0' + nextValue.getDate() : nextValue.getDate());

        return `${year}-${month}-${date}`;
    }

    nextDate.value = setNextDate(ft.selectedDates[0], interval);
    // End Set Next Date Valu
    ...
    //    });
       ...
       interval.addEventListener('change', e => {
            nextDate.value = setNextDate(ft.selectedDates[0], e.target);
