...
<div class="row h-100 justify-content-center align-items-center">
    <div class="col-12 col-md-8 col-lg-5">
        <div class="card fadeIn3 fadeInBottom">
            ...
            <div class="card-body">
                <form name="form-input" action="{{ route('users.update', ['user' => $user]) }}" class="d-flex flex-column" method="post">
                    @csrf
                    @method('PUT')
                    <span class="input-group input-group-outline @error('username') is-invalid @enderror">
                        ...
                        <input type="text" class="form-control" name="username" value="{{ old('username') ?? $user['username'] }}" @error('username') autofocus @enderror/>
                    </span>
                    @error('username')
                    ...
                    <span class="input-group input-group-outline @error('email') is-invalid @enderror mt-3">
                        ...
                        <input type="email" class="form-control" name="email" value="{{ old('email') ?? $user['email'] }}" @error('email') autofocus @enderror/>
                    </span>
                    @error('email')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                    ...
                    <span class="input-group input-group-outline @error('fullname') is-invalid @enderror mt-3">
                        ...
                        <input type="text" class="form-control" name="fullname" value="{{ old('fullname') ?? $user['fullname'] }}" @error('fullname') autofocus @enderror/>
                    </span>
                    @error('fullname')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
