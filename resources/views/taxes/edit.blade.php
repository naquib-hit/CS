...
<div class="row h-100 justify-content-center align-items-center">
    ...
    <div class="col-12 col-md-8 col-lg-5">
        <div class="card fadeIn3 fadeInBottom">
            ...
            <div class="card-body">
                <form name="form-input" action="{{ route('taxes.update', ['tax' => $tax]) }}" class="d-flex flex-column" method="post">
                    @csrf
                    @method('PUT')
                    <span class="input-group input-group-outline @error('tax_name') is-invalid @enderror mt-3">
                        ...
                        <input type="text" class="form-control" name="tax_name" value="{{ old('tax_name') ?? $tax->tax_name }}" @error('tax_name') autofocus @enderror/>
                    </span>
                    @error('tax_name')
                    ...
                    <span class="input-group input-group-outline @error('tax_amount') is-invalid @enderror mt-3">
                        ...
                        <input type="number" class="form-control" name="tax_amount" value="{{ old('tax_amount') ?? $tax->tax_amount }}" @error('tax_amount') autofocus @enderror/>
                    </span>
                    @error('tax_amount')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
